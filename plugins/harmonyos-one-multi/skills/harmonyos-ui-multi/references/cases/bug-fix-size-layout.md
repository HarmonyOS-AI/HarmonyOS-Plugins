...
完整案例对照（goodcase: List 横向滚动 vs badcase: Row 截断）见 [list-scroll-extension-avoid-truncation.ets](../../assets/size-layout/list-scroll-extension-avoid-truncation.ets)。
...
完整案例对照（goodcase: flexShrink 精确分配 vs badcase: 文本溢出截断）见 [row-flexshrink-text-ellipsis.ets](../../assets/size-layout/row-flexshrink-text-ellipsis.ets)。
...
`displayPriority` 已经是可见数量的唯一事实源：始终把完整 `items` 交给 `ForEach`，让框架按实际
测量宽度决定隐藏哪些子组件。不要同时通过 `windowSizeChange` / `onAreaChange` 维护容器宽度，
不要用 `availableWidth / itemWidth` 推导 `visibleCount`，也不要先 `slice` / `filter` 数据。
这些计算既重复，又无法可靠覆盖变长文本、字体缩放、组件间距、内边距和运行时数据变化。

相同显示优先级的子组件会同时显示或隐藏，不会按组件排列位置从尾部逐项处理。小数优先级落在同一
区间 `[x, x + 1)` 时也视为同一优先级。因此，“靠后元素先隐藏”只能来自代码为靠后元素显式分配了
更低优先级，不能当作同优先级时的框架行为。

多个元素需要保持整体显隐时，应先封装成一个直接子组件，再在组容器上设置 `displayPriority`，以明确
原子显隐边界。不能因为担心“同优先级从尾部逐项隐藏”而排除 `displayPriority`；该行为并不存在。
只有产品要求固定显示 N 个，或隐藏项必须进入“更多”入口时，才改用手动分组/计数并移除
`displayPriority`。

完整案例对照（goodcase: displayPriority 按优先级隐藏 vs badcase: 标签截断）见 [row-displaypriority-truncation.ets](../../assets/size-layout/row-displaypriority-truncation.ets)。
...
Row({ space: 12 }) {
  ForEach(items, (item, index) => {
    Text(item)
      .displayPriority(items.length - index)
  })
}
  .width('100%')
```

如果业务要求固定显示 N 个，或需要把隐藏项放入“更多”菜单，应改用手动分组/计数方案，并移除
`displayPriority`；不要让两套机制共同控制同一批子组件。
...
| **容器要求** | 必须设置主轴尺寸约束（Row 设 width，Column 设 height） | 仅 Flex 单行模式生效（Row 或 Flex 未设 wrap），容器必须设置 `.width('100%')` 或固定宽度；完整渲染数据，不另算可见数量 |
...
7. 宽度断点有阶梯变化时（如手机→平板），可结合方案一与方案四：用 Scroll 兜底防截断，同时按断点调整内容区高度比例。
...
display.on('change', (data: number) => {
  ...
  this.calcContentSize(defaultDisplay.width, defaultDisplay.height);
});
```
...
this.hostWindow.on('windowSizeChange', (size: window.Size) => {
  ...
  this.calcContentSize(size.width, size.height);
});
```
...
8. **检查 flex 容器溢出截断**：`Row`/`Column`/`Flex` 内子组件被截断或意外压缩时，检查是否正确配置了 `flexShrink`（控制谁被压缩）和 `displayPriority`（控制谁被隐藏，仅在 Flex 单行模式生效），并确保容器设置了主轴尺寸约束。已经使用 `displayPriority` 时还要检查并移除基于宽度推导 `visibleCount`、再 `slice` / `filter` 子元素的重复显隐逻辑。详见 SIZE-04。
...
未根据断点动态调整布局和设置小窗口布局。可复用实现见 [SplitScreenAbility.ets](../../assets/window-form/SplitScreenAbility.ets)、[SplitScreenMainPage.ets](../../assets/window-form/SplitScreenMainPage.ets) 和 [SplitScreenDetailPage.ets](../../assets/window-form/SplitScreenDetailPage.ets)。
...
- 三分栏场景（SideBarContainer + Navigation）的正确嵌套方式见 [size-layout.md 的“分栏布局要点”](../../domains/size-layout.md#分栏布局要点)。

---

### LAYOUT-07 日历应用侧边 Tabs 页签间距过小

#### 真实案例

以下代码直接取自某日历应用的 `TabContainer` 页，完整保留与 Tabs
直接相关的 `tabBuilder`、三个 `TabContent` 和 Tabs 属性链，不再改写成通用伪代码：
...
build() {
  Column() {
    ...
    .barBackgroundColor(this.theme.themeState.current)
...
- `top: 5` 是该应用的页面设计值，不是 HarmonyOS 通用推荐值。
- `bottomRectHeight` 由该项目的 `WindowUtils.ets` 从
  `AvoidAreaType.TYPE_NAVIGATION_INDICATOR` 读取后写入 `AppStorage`，属于安全区状态，不是固定页签间距。
- 修复其他项目时，在 LG 侧边栏下为每个自定义 `tabBar` Builder 的页签项根节点同时设置
  `top`、`bottom` margin；模型结合页签内容尺寸、侧栏可用高度和视觉效果判断数值并验证，
  不要求项目预先存在对应资源，也不机械照抄案例数值。
- 页签较多时仍由 `BarMode.Scrollable` 处理溢出，不按页签数量或侧栏剩余高度动态计算间距。
...
7. **检查侧边 Tabs 的页签项间距**：LG 下每个自定义 `tabBar` Builder 的页签项根节点必须同时设置 `top`、`bottom` margin（详见 LAYOUT-07）。
