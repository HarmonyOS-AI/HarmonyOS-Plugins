...
4. [场景4：顶部和底部 padding 位置错误](#场景4顶部-padding-位置错误导致背景色间隙--底部-padding-位置错误导致列表无法沉浸式滚动)
...
13. [场景13：自定义键盘在宽屏/展开态留白与整体过高](#场景13自定义键盘在宽屏展开态留白与整体过高)
...
> **实现对照：** 本节内联代码对比错误用法（`display.on('change')` 中获取避让区）与正确用法（`window.on('avoidAreaChange')`），不依赖仓库外示例。
...
## 场景13：自定义键盘在宽屏/展开态留白与整体过高

> **实现对照：** 本节内联代码对比错误用法（builder 内硬编码 width/height + 左对齐）与正确用法（自适应宽度 + `layoutWeight` 填满 + 按屏高动态算高度），不依赖仓库外示例。

**问题描述：**
- 自定义安全键盘通过 `.customKeyboard(builder, { supportAvoidance: true })` 挂在 `TextInput` 上
- 折叠屏展开态（内屏 ~711vp）键盘右侧出现大片留白，手机态（~374vp）正常
- 键盘底部留白、按键没填满；或整体高度过高，占屏比过大

**根因分析：**
1. builder 里给标题栏 / 按键 Grid 硬编码了固定宽度（如 `width(459)`，常是某机型外屏宽），外层 `Column` 又是 `width('100%')` 且 `alignItems(HorizontalAlign.Start)`。宽屏面板比内容宽，左对齐后右侧空一大块；手机态面板更窄、内容被裁剪或刚好填满，所以右侧留白只在宽屏暴露
2. 按键 Grid 用固定 `height`，根 `Column` 高度大于内容总高，`justifyContent` 默认 Start，空隙堆在底部
3. 根组件写死 `height`，不随屏高变化。不同设备/形态屏高不同，写死值无法兼顾，表现为整体过高或过矮
...
export struct QuickLoginPage {
  // ❌ 取了宽度却没取高度，且只在 aboutToAppear 取一次
  @Local displayW: number = 0
  ...
  getDisplayWidth() {
    ...
    this.displayW = this.getUIContext().px2vp(d.width)
  }

  @Builder
  phoneKeyboard() {
    Column() {
      Row() { Text('安全键盘'); Text('完成') }
        .width(459)                  // ❌ 硬编码设备宽度
        .height(44)
      Grid() { /* 4×3 数字键 */ }
        .height(264)                 // ❌ 固定高度，填不满根 Column
        .width(459)                  // ❌ 硬编码设备宽度
    }
    .width('100%')
    .alignItems(HorizontalAlign.Start)   // ❌ 左对齐 → 右侧留白
    .height(400)                       // ❌ 写死高度，不随屏高适配
  }
}
```

**解决方案：** 宽度去硬编码改自适应，内容区用 `layoutWeight(1)` 填满高度，键盘总高按屏高动态计算。三个症状各有对症修法，不要混用。
...
export struct QuickLoginPage {
  ...
  getDisplaySize() {
    const d = display.getDefaultDisplaySync()
    // ✅ height 是 px，参与 vp 运算前先 px2vp()
    this.displayH = this.getUIContext().px2vp(d.height)
  }

  // ✅ 键盘高度按屏高比例算（如屏高 / 2），不写死
  @Computed
  get keyboardHeight(): number {
    return this.displayH > 0 ? this.displayH / 2 : 400
  }
}
```

**关键点：**
- `display.getDefaultDisplaySync().height` 返回 **px**，与 vp 阈值/算术比较前必须 `px2vp()`
- 比例由产品定（屏高 / 2、/ 3 等），不要写死 `height(400)`；`displayH` 未就绪时给一个安全兜底值
- `aboutToAppear` 只取一次；用户停留期间折展/旋转导致屏高变化不会自动重算。要实时跟随需再挂 `windowSizeChange`（或 `display.on('change')`，注意它回调里属性可能未刷新）重算
  ...
  phoneKeyboard() {
    Column() {
      Row() { Text('安全键盘'); Text('完成') }
        .width('100%')              // ✅ 自适应面板宽度
        .height(44)
      Grid() { /* 4×3 数字键，columnsTemplate('1fr 1fr 1fr') */ }
        .layoutWeight(1)            // ✅ 填满标题栏以下剩余高度，消除底部留白
        .width('100%')              // ✅ 自适应面板宽度
    }
    .width('100%')
    // ✅ 去掉 alignItems(HorizontalAlign.Start)
    .height(this.keyboardHeight)    // ✅ 按屏高动态
    .backgroundColor('#DEE0E6')
  }
```

**关键点：**
- `.customKeyboard` 的键盘面板宽度 = builder 根组件宽度。根组件 `width('100%')` 时面板在展开态可达 ~711vp；内部内容也必须 `width('100%')` 铺满，否则宽屏右侧必然留白
- 三个独立症状各有对应修法，不要混用：右侧留白 → 去硬编码宽 + 去左对齐；底部留白 → 内容区 `layoutWeight(1)`；整体过高 → 高度按屏高动态算
- 自定义键盘的标准形态就是铺满面板宽度（`width('100%')`），直接这么做即可，不必询问用户、也不必提供居中限宽方案
- 根因索引见 `root-causes.md` 的 avoid-12
