...
| 模式 | 什么时候用 | 组件 | 关键属性 |
|---|---|---|---|
| **重复** | 同质内容随空间增减列数 | `List` / `WaterFlow` / `Grid` / `Swiper` | `lanes` / `columnsTemplate` / `displayCount` |
...
判断顺序：内容是否同质 → 是否有主从关系 → 是否只是方向变化 → 是否有明确的阅读行长或表单宽度约束。最后一项不成立时不要用缩进兜底；保持全宽并继续分析页面语义，不能把手机单列简单居中后结束。注意"有主从关系"只是分栏的必要条件而非充分条件：未过「分栏布局要点」的前提门槛时，用重复、缩进兜底，不引入分栏。
...
1. **`breakpoints.value` 必须是完整四值数组** `['0', '600vp', '840vp', '1440vp']`，**不能截断成两个**。数组里有几个值，就只有那几档断点会触发：只写 `['600vp', '840vp']` 时 lg / xl 两档**永不触发**，`GridCol` 里给 `lg`、`xl` 设的 `span` 静默失效，大屏一直停在 md 的列数。开头的 `'0'` 把 xs 区间压成空、让有效档位从 sm 起算。

   ❌ `value: ['600vp', '840vp']` —— 只剩 sm/md 两档边界，lg 以上不分档
   ✅ `value: ['0', '600vp', '840vp', '1440vp']` —— sm/md/lg/xl 四档齐
...
GridRow({
  breakpoints: {
    value: ['0', '600vp', '840vp', '1440vp'], // 四个边界写全，不可截断
...
2. **断点值数组被截断**：`value` 只写了 `['600vp','840vp']` 两值，lg/xl 两档**永不触发**、对应 `GridCol span` 静默失效。必须是完整四值 `['0','600vp','840vp','1440vp']`（详见上文「GridRow 配置」）
