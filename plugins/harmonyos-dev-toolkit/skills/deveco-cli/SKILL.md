...
name: deveco-cli
description: >-
  Use `devecocli` when actively developing or maintaining a HarmonyOS application: scaffolding or changing a project, building, running, debugging, managing development devices or emulators, inspecting app logs, or looking up documentation needed to implement or troubleshoot the app. Trigger for a HarmonyOS workspace only when the current request concerns that application's development. Do not trigger for general discussion, news, conceptual questions, product comparisons, or a mere mention of HarmonyOS, 鸿蒙, DevEco, ArkTS, ArkUI, documentation, search, knowledge, or skills.
---
...
Run commands within the host's available execution permissions. If a build or device operation is blocked by sandbox restrictions, use the host's supported permission flow; never bypass a denial.

## Prerequisites and official documentation

Confirm `devecocli --version` and use `devecocli <command> --help` to check options against the installed version. The plugin supplies this skill, not the CLI binary or HarmonyOS SDK. If the CLI is missing, use Huawei's [AI Coding resource page](https://developer.huawei.com/consumer/cn/harmonyos-ai).

The `harmonyos-dev-toolkit` plugin configures Huawei's official remote documentation service as `KnowledgeMCP`. For current API signatures, permissions, version constraints, and development guidance:

- Discover the connected server's tools and their schemas; client prefixes may differ.
- Use `searchDocuments` to locate relevant official documentation.
- Pass the returned `parent` identifiers to `getDocumentsById` through its `names` field (up to 10 documents), using the request wrapper required by the discovered schema when full context is needed. Cite the returned official document links and check their API levels against the project.
- If the service is unavailable, use `devecocli docs` for local documentation or browse the official site. State the source and any version limitations.

See Huawei's [developer knowledge MCP guide](https://developer.huawei.com/consumer/cn/doc/start/hosknowledgemcp-0000002664603963). This remote documentation service is separate from the local syntax-checking server configured by `devecocli init --mcp`.
...
- **`devecocli update`**: Update CLI to latest version.
