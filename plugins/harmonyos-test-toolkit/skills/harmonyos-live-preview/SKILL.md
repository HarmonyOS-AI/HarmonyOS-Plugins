...
需要在真机或模拟器上验证应用导航、系统弹窗、设备能力或完整业务流程时，可使用同插件提供的
`harmonyrun` Skill。Previewer 的截图和组件树只能证明预览环境中的结果；
涉及真实运行时的测试还需在设备上验证。
...
以本次实际读取的 `SKILL.md` 所在目录作为 `PREVIEW_SKILL_DIR`，使用绝对路径。
从宿主提供的技能位置解析它；不要根据插件名、缓存版本或当前工程目录猜路径。
插件升级后旧缓存可能被删除，路径失效时重新获取技能位置。

下面的占位路径必须替换为实际目录；Shell 命令在 Bash / Zsh 中通用，每个新 Shell 会话先执行：

```bash
export PREVIEW_SKILL_DIR="/absolute/path/to/harmonyos-live-preview"
test -f "$PREVIEW_SKILL_DIR/scripts/preview.mjs" && \
  test -f "$PREVIEW_SKILL_DIR/scripts/drive.mjs"
drive() { node "${PREVIEW_SKILL_DIR:?先设置实际技能目录}/scripts/drive.mjs" "$@"; }
```

两个文件检查都通过后再启动。`drive` 是 Shell 函数，用于下文的验证命令，支持含空格的安装路径。
这是本技能自行设置的变量，不依赖宿主注入 `CLAUDE_PLUGIN_ROOT`。
...
```bash
node "${PREVIEW_SKILL_DIR:?先设置实际技能目录}/scripts/preview.mjs" \
  ...
  "configurations": [
    {
      ...
      "runtimeArgs": [
        "/absolute/path/to/harmonyos-live-preview/scripts/preview.mjs",
        "--project", "/absolute/path/to/HarmonyOSProject"
      ],
      "port": 8088
    }
...
将 `runtimeArgs` 的第一个值替换为已验证的脚本绝对路径；JSON 配置不要依赖 Shell 变量展开。
